...
int main() {
  ...
  // op type
  const std::string op_type = "DeformableConv2D";
  const int64_t kernel_h = 3;
  const int64_t kernel_w = 3;
  // input - x
  const std::vector<int64_t> x_dims{2,8,4,4};
  const int64_t x_numel = get_numel(x_dims);
  std::vector<float> x_data(x_numel);
  std::iota(x_data.begin(), x_data.end(), 0);
  // input - offset
  const std::vector<int64_t> offset_dims{2, 3 * kernel_h * kernel_w, 4, 4};
  const int64_t offset_numel = get_numel(offset_dims);
  const std::vector<float> offset_data(offset_numel, 0.0);
  // input - filter
  std::vector<int64_t> filter_dims{4,8,3,3};
  int64_t filter_numel = get_numel(filter_dims);
  std::vector<float> filter_data(filter_numel);
  std::iota(filter_data.begin(), filter_data.end(), 0);
  // output
  const std::vector<int64_t> y_dims{2,4,4,4};
  ...
  const std::vector<int64_t> pads{1, 1, 1, 1};
  ...
  auto input_filter = new npuTensor<float>(ACL_FLOAT, filter_dims.size(), filter_dims.data(), ACL_FORMAT_NCHW, filter_data.data());
  // set inputs desc and buffer
  std::vector<aclTensorDesc *> input_descs;
  std::vector<aclDataBuffer *> input_buffers;
  input_descs.emplace_back(input_x->desc);
   input_descs.emplace_back(input_filter->desc);
  input_descs.emplace_back(input_offset->desc);
 
  input_buffers.emplace_back(input_x->buffer);
   input_buffers.emplace_back(input_filter->buffer);
...
//   ACL_CALL(aclopSetAttrListInt(attr, "ksize", ksize.size(), ksize.data()));
  ...
  ACL_CALL(aclopSetAttrListInt(attr, "dilations", dilations.size(), dilations.data()));
   ACL_CALL(aclopSetAttrInt(attr, "groups", 1));
